...
pub async fn get_deposit_status(
    ...
    let wallets = app_state.wallets.read().unwrap();
    let user_id = user_id.into_inner();
    let (_, chain) = wallets.get(&user_id)
        .ok_or_else(|| PulserError::UserNotFound(format!("User {} not found", user_id)))?;
    Ok(HttpResponse::Ok().json(serde_json::json!({
        "user_id": chain.user_id,
        "address": chain.multisig_addr,
        "btc_balance": chain.accumulated_btc.to_string(),
        "usd_balance": chain.stabilized_usd.0, // Static total
...
pub async fn create_deposit(
    ...
    let config = app_state.config.read().unwrap();
    let (wallet, deposit_info, mut stable_chain) = DepositWallet::create_for_user(
        &user_id.to_string(),
        "config/service_config.toml",
        &config.lsp_pubkey,
        &config.trustee_pubkey,
        &req.user_pubkey.clone().unwrap_or_default(),
        app_state.network,
    )?;
    stable_chain.raw_btc_usd = *app_state.current_price.read().unwrap();
    stable_chain.synthetic_price = Some(*app_state.synthetic_price.read().unwrap());
    stable_chain.expected_usd = USD(req.expected_amount_usd.unwrap_or(0.0));
    ...
    let response = serde_json::json!({
        ...
        "expected_amount_usd": stable_chain.expected_usd.0,
        "current_btc_price": stable_chain.raw_btc_usd,
        "synthetic_price": stable_chain.synthetic_price.unwrap_or(0.0),
        "created_at": stable_chain.formatted_datetime,
        "network": config.network,
        "deposit_info": deposit_info,
    });
    info!("Created deposit address for user {}: {}", user_id, deposit_info.address);
    Ok(HttpResponse::Ok().json(response))
}
